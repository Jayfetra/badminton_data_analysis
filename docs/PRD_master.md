...
**Version:** 3.1 (Iteration 12: deep-dive analysis R9 and matches not played yet; R1-R9 complete) · **Last updated:** 2026-09-21
...
| R9 | Deep dive into one player's saved year (section 12): tournaments entered, rests and time on tour, how often each round was reached, wins in two and in three games, and the correlation between the longest run of consecutive points and winning. **Done (Iteration 12).** |
...
| `h2h/match` | `tmt_id` (tournament id), `match_code` (the `code` of the match in `vue-player-tmt-matches`) | **R8 source: everything on a match page** (`.../tournament/5515/<slug>/match/13`): `tournament{name,id}`, `info{drawName,roundName,winner,scoreStatus}`, `matchStartTimeDetails{dateLocal,dateTimeLocal}`, `location{locationName}`, `progress{duration}`, `team1`/`team2` (`player1`, `player2`: id, `name_display_bold` as HTML, slug, `nationality_item.name`), **`stats`** = the Match tab (`team1_/team2_` `result`, `consecutive_points`, `game_points`, `rallies_played`, `rallies_won`, plus `smash_winner`, `net_winner`, `clear_winner`, `other`, `challenge_used/won/lost/nodecision`, `tournament_match_id`) and **`games[]`** = the Game tabs (`ordering`, `team1`, `team2`, `total_points_played`, `match_set_stats_model` with the same statistics, and **`match_set_details_model`: the cumulative score after every rally**, `{team1, team2, ordering}`). Same session cookie and Referer as the other calls (the existing client works with the endpoint `h2h/match`). The response's `tournament_match_id` equals the match id we already hold (88 of 88 checked) and `code` is unique within a tournament. **A match that does not exist gives HTTP 404 with `{"stats":null,"games":[]}`.** Byes and walkovers answer with `games: []`. **Coverage varies:** World Tour level events have the rally sequence and all statistics; lower-level events (International Challenge, ...) have only the game scores, with `match_set_details_model: []` and every statistic `0` (meaning not tracked). The tracking fields (smash/net/clear winners, challenges) were null or 0 in every match seen. | Used (Iteration 9) |
...
4. **Live testing.** The block observed in Iteration 0 lifted within a day. Since then the live smoke suite (20 tests, 4 min 43 s after Iteration 12, 4 min 38 s after Iteration 11, because it now does the real download of about 85 requests; 2.5 minutes after Iteration 10, 18 tests after Iteration 9, 15 after Iteration 8; earlier versions of this line said 8 tests / 40 s, which had already gone stale at 9 tests in Iteration 4, then 11 in Iteration 5; requests are 2.5 s apart, custom `Mozilla/5.0 (compatible; bwf-player-lookup/...)` User-Agent) and repeated notebook runs have caused no block. Run the live suite sparingly. **Live tests must not assume that the player's latest tournament is a finished one**: on 2026-09-25 it was an event under way with only a bye and a scheduled match, and three live tests that used "the latest tournament" failed (the code was right, the assumption was not); they now use fixed, finished tournaments.
...
bwf_player/
  ...
  exceptions.py   BwfClientError, BlockedByCloudflareError, BwfNotFoundError (HTTP 404), InvalidInputError
  models.py       PlayerCandidate, SearchResult, PlayerProfile, PlayerRanking, PlayerResult, TournamentEntry, TournamentHistory, PlayerMatch, GameScore, MatchPlayer, EventMatches, HistorySummary, MatchDetails, GameDetail, SideStats, Rally, DetailPlayer
  ...
  parsing.py      to_int, to_date, clean_text, to_code: defensive coercion shared by the parsers
  game_details.py R8a: get_match_details (Match tab + Game tabs of one match), the derivations and checks, format_match_details (Iteration 9)
  analysis.py     R9: count_tournaments, activity, round_progress, game_split, run_correlation, deep_dive and their text (Iteration 12)
  history.py      download_player_history (search -> tournaments -> matches -> store -> export) and format_history (Iteration 8)
  store.py        R7/R8b: HistoryStore (SQLite tables, views, migration, CSV export; game details since Iteration 10)
...
- **Game details (R8a, implemented in Iteration 9).** `game_details.get_match_details(tournament_id, match_code, client=None, *, match=None)` makes one request to `h2h/match` and returns a `MatchDetails`. `details_targets(matches)` picks the matches worth requesting (played or retired, with a `match_code`; byes, walkovers, disqualified and unknown are skipped without a request). `format_match_details` prints the Match tab and the Game tabs as the site lays them out.
  - *Orientation*: the site's own (side 1 = team 1), as in `vue-player-tmt-matches`, so a stored match is identical whichever player it was downloaded for; the per-player flip happens in the views (Iteration 10).
  - *Three levels of coverage*: full (rally sequence and all statistics); **games only** (lower-level events: the site sends zeros that mean "not tracked", returned as `None` with `tracked=False` and a note, never as real zeros); none (byes, walkovers).
  - *Everything is re-derived and checked* (`check_internal`; each check is tested by corrupting a real response). Per tracked game: the number of rallies equals `total_points_played` and the points scored; the sequence ends at the game score; every rally adds exactly one point to one side; the rally numbers run 1..n; the site's `consecutive_points` equals the longest run of points in the sequence; `game_points` equals the number of rallies played while the side was one point from the game (at least 20 points and ahead, or 29); `rallies_played` equals the rally count and `rallies_won` the side's points. Per match: **the longest run crosses game boundaries** (side 2's run of 6 in match 5227/3 beats its best inside any game, 4), `game_points`, `rallies_played` and `rallies_won` are sums over the games, and the final result equals the games won (not for a retirement).
  - *Checked against our own data* (`check_against_match`, when a `PlayerMatch` is given): same match id, tournament, players (by id, on the right sides), game scores (0-0 games excluded) and winner. The names on the match page can differ slightly from the player page (`Bernadine Anindya WARDANA` v `Bernadine Anindiya WARDANA`), which is why players are compared by id.
...
- **Matches not played yet (Iteration 12).** `match_state` "F" means finished (no state also counts as finished). For a `played` match with any other state the status becomes `scheduled` (no games yet) or `in_progress` (games so far); `won` stays None, no "no usable winner" note is written, the report prints `not played yet (date)`, `details_targets` skips it, the store keeps it (`winner_side` NULL) and the analysis shows it as `pending` in the round funnel and ignores it in the calendar. The site's own totals count only finished matches (and a bye as a win), so `check_totals` still agrees.
- **Deep-dive analysis (R9, Iteration 12).** See section 12: `bwf_player.analysis` reads the saved database (tables and views) for one player and a date window, and answers five questions with tested definitions. It never calls the site.
...
- `PlayerMatch` (R5, R6): `match_id`, `match_code` (Iteration 9), `tournament_id`, `draw_id`, `draw_name`, `round`, `match_date`, `duration_min`, `side`, `player`, `partner`, `opponents`, `won`, `status`, `games`, `notes`
- `EventMatches` (R5, R6): `tournament_id`, `event_code`, `event_id`, `matches`, `totals_agree`, `notes`
- `SideStats` (R8): `consecutive_points`, `game_points`, `rallies_played`, `rallies_won`, `smash_winner`, `net_winner`, `clear_winner`, `other`, `challenge_used`, `challenge_won`, `challenge_lost`, `challenge_nodecision`
- `Rally` (R8): `rally_no`, `side1_points`, `side2_points`, `winner_side`
...
- Analysis results (R9, `bwf_player/analysis.py`): `TournamentCount`, `Activity` (`TournamentSpan`, `RestPeriod`), `RoundProgress` (`RoundCount`, `GroupOrTeamEvent`), `GameSplit`, `RunCorrelation`, `DeepDive`; `MatchStatus` also has `scheduled` and `in_progress` (Iteration 12)
...
| 12 | R9: deep-dive analysis module and notebook section 7; matches not played yet (`scheduled`, `in_progress`) | Done |
...
- `matches(match_id PK, tournament_id, event_id, event_code, seq, draw_id, draw_name, round, match_date, duration_min, status, winner_side)`. `status` is one of played, bye, walkover, retired, disqualified, scheduled, in_progress, unknown. `winner_side` is 1 or 2, NULL for a bye or an unknown winner. `seq` is the match's position within its event, in playing order.
...
## 11. Game details of every match (R8)

Added 2026-09-21. For each match downloaded in section 10, record what the site's match page shows (for example `bwfworldtour.bwfbadminton.com/tournament/5515/all-england-open-badminton-championships-2026/match/13`): the **Match** tab and one tab per game (**Game 1**, **Game 2**, and Game 3 if played).

### Decisions (user, 2026-09-21)

- **Scope**: only the input player's own matches, in the same one-year window as the history (not every match of every tournament). Fetching every match of the tournaments would be roughly 2,000-3,000 requests for one player instead of about 60.
- **Default**: the download fetches the details automatically; `game_details=False` (command line `--no-game-details`) skips them and keeps the older, shorter download.
- "All information ... recorded correctly": every field the site sends is kept, including the ones the page does not show (challenge counts, smash/net/clear winners; empty in all matches seen), and everything is checked (section 4, "Game details").

### What the match page contains

- **Match tab**: final match score, each game's score, game points, most consecutive points, total points played, total points won, per side.
- **Game tabs**: score, most consecutive points, game points, total points played, total points won, and a line chart of the **score after every rally**.
- Also on the page: tournament, draw, round, start time, venue, duration, the players.
- Example: All England 2026, R16 (match code 13, our match id 1505450): LIN Chun-Yi (side 1) beat Jonatan CHRISTIE (side 2) 21-19, 21-12 in 48 minutes at the Utilita Arena Birmingham. Match tab: result 2-0, game points 7-0, most consecutive points 7-5, total points played 73-73, won 42-31. Game 1: 40 rallies (21-19), Game 2: 33 rallies (21-12).

### Data model (implemented in Iteration 10; SQLite schema version 2)

- `matches` gains `match_code` (the number in the match page URL). A later save without a code never erases a stored one; saving the details fills in a missing one.
- `match_stats(match_id PK, start_local, venue, score_status, tracked, checks_ok, differences, side1_result, side2_result, side1_/side2_ x 12 statistics)`: the **Match tab**, one row per match whose details were saved. The 12 statistics per side are the `SideStats` fields: `consecutive_points`, `game_points`, `rallies_played`, `rallies_won`, `smash_winner`, `net_winner`, `clear_winner`, `other`, `challenge_used`, `challenge_won`, `challenge_lost`, `challenge_nodecision`. `tracked` is 0/1, `checks_ok` is 1, 0 or NULL, `differences` holds the failed checks as text.
- `game_stats(match_id, game_no, total_points_played, tracked, side1_/side2_ x 12 statistics)`: the **Game tabs**, one row per game.
- `rallies(match_id, game_no, rally_no, side1_points, side2_points, winner_side)`: the **score after every rally**, the data behind the page's chart.
- Everything in the site's orientation (side 1 = team 1). **Statistics the site does not track are NULL, never 0**: for a games-only match, `tracked = 0`, every statistic is NULL and there are no rally rows. The column lists of the DDL, the inserts, the views and the CSV files are generated from `SideStats`, so they cannot drift apart.
- Views in the player's point of view (`player_` and `opponent_` columns, the player's figures first, for players whose own history was saved): `player_match_stats_view` (the Match tab, incl. `player_games_won`), `player_game_view` (one row per game, joined to the game's score; games without saved details show NULL statistics) and `player_rally_view` (the progression with `rally_won_by_player`).
- CSV: `match_stats.csv`, `game_stats.csv`, `rallies.csv` with the same perspective; `match_code` is now the last column of `matches.csv`. A header-only file when no details were saved.
- **Migration 1 -> 2** happens when an older database is opened: `matches.match_code` is added, the new tables and views are created, the version is set to 2; nothing is deleted or rewritten (tested by turning a real database back into a version 1 file). `match_code` of already stored matches stays NULL until the matches are saved again (the normal download does that). A database from a newer version is still refused.
- `HistoryStore.save_match_details(details)`: the match must be stored already and belong to the same tournament, else `InvalidInputError`; one transaction; replaces everything stored for the match (tested with a rollback on a constraint failure and by saving twice).
...
## 12. Deep-dive analysis of one player's year (R9)

Added 2026-09-25. The owner asked, starting with Jonatan Christie: (1) how many tournaments in a year, (2) how often he rested and how long he was at tournaments in the one-year cycle, (3) how many times he reached each round (R64, R32, R16, QF, SF, Final, champion), (4) how many matches he won in two games and in three, (5) whether the most consecutive points correlates with winning. Implemented as `bwf_player/analysis.py` (read-only on the saved database, parameterised by player and window) and notebook section 7 (the player of section 4; change `HISTORY_PLAYER` to dive into someone else).

### Definitions

- **Window**: the download's window. A tournament belongs to it when its dates overlap it (the same rule as the download), so it counts in question 1 even if all its matches were outside.
- **On tour / rest** (question 2): the player is on tour from the first to the last day of the player's matches at a tournament. A **rest** is a stretch of full days between the last match of one tournament and the first of the next. Only tournaments where the player played a match inside the window count; overlapping tournaments (a team event next to another) are merged so days are never counted twice; a tournament with only a bye and a scheduled match is left out with a note. Scheduled days (the calendar days the tournaments run) are shown next to the days actually on tour, because the player leaves when knocked out. Only BWF tournaments are known, so other competitions inside a rest are not seen.
- **Rounds** (question 3): for individual knockout events only, a round is *reached* when the event has a match (or a bye) in that round; `reached = won + lost + pending` in every round, and the wins of one round equal the events reaching the next. A bye counts as reaching and winning; **champion** = won the final. Team events and group stages are listed apart (they have no such rounds); qualifying rounds are counted separately.
- **Games** (question 4): played best-of-three matches only (retirements left out): won 2-0 and 2-1, lost 1-2 and 0-2, plus how often the match was won after winning or after losing the first game.
- **Runs and winning** (question 5): only matches (and, separately, games) with rally-by-rally data. The run difference is the player's longest run of consecutive points minus the opponent's. Reported: the win rate when the run was longer, equal or shorter; the correlation `r` between the run difference and winning (a point-biserial, i.e. Pearson with a 0/1 outcome); a **permutation p-value** (the outcomes shuffled 10,000 times with a fixed seed, so the number is reproducible); the correlation of the run difference with the points balance; and the **partial correlation** of run difference and winning with the points balance taken out. A correlation is not causation: a longer run usually goes with more points overall, which wins by itself.

### Results for Jonatan CHRISTIE (window 2025-09-26 to 2026-09-26, the committed notebook run, real data)

| Question | Answer |
|---|---|
| 1. Tournaments | **20** (18 individual, 2 team; 20 events). By category: World Tour Super 750: 6, Super 500: 4, Super 1000: 4, World Tour Finals: 1, plus the World Championships, Asian Championships, Thomas Cup, and the Asian Games (individual and team). |
| 2. Rest and time on tour | On tour **61 days = 17%** of the 366-day window (57 days with a match; the 19 tournaments played run 114 scheduled days, he stayed 3.2 days on average). The window is rolling: a day earlier it was 62 days, because the Korea Open of 24-28 Sep 2025 then had one more day inside it. **18 rests** between tournaments: average 16.8 days, median 16, longest **44 days** (19 Jan to 3 Mar). By length: 1-6 days: 5, 7-13: 3, 14-27: 7, 28 or more: 3. No back-to-back tournaments without a rest day. |
| 3. Rounds (17 individual knockout events) | R64: 2 (2 won). R32: **17** (13 won, 3 lost, 1 not played yet). R16: **13** (9 won, 4 lost). QF: **9** (6, 3). SF: **6** (5, 1). Final: **5** (3 won, 2 lost): **champion 3 times** (Korea Open, Denmark Open, HYLO Open), runner-up twice. Team events and group stage: Asian Games team 0-3, Thomas Cup 1-2, World Tour Finals group 0-3. |
| 4. Games | 59 played matches: **won 38: 22 in two games, 16 in three**; lost 21: 7 in three games, 14 in two. Three-game matches: 23, won 16 (70%). After winning game 1 he won the match 28 of 34 (82%); after losing game 1, 10 of 25 (40%). |
| 5. Runs and winning | **Yes, positive.** 56 matches: win rate 86.7% when his run was longer (26 of 30), 60% when equal (3 of 5), 42.9% when shorter (9 of 21); r = **0.46** (moderate), permutation p = 0.0003. 134 games: 94.1% / 66.7% / 12.5%; r = **0.69** (strong), p < 0.0001. **But it is mostly the points:** the run difference correlates 0.70 (matches) and 0.82 (games) with the points balance, and with the points balance taken out the partial correlation is -0.33 for matches and -0.09 for games, i.e. the run adds nothing beyond the points (the negative sign at match level should not be over-read with 56 matches). |

### Verification

Every answer is recomputed in the tests by a route that does not use the module's SQL: from the parsed matches and match details in the fixtures (a day-by-day calendar for the rests, a counter over the parsed rounds, the games of each match, a hand-written Pearson formula). Synthetic databases with hand-worked numbers pin down each definition and edge case (back-to-back and overlapping tournaments, clipping at the window edges, byes, walkovers, qualifying, group and team events, pending matches, perfect and absent correlations, too few matches, one player's data never leaking into another's). The notebook guard checks that the printed round table adds up.

### Limits

One player and one year is a small sample (a few dozen matches): differences of a few percentage points mean little. Rests only see BWF tournaments. Only matches with rally data enter question 5 (three Asian Games matches have game scores only).
